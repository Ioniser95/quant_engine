...
import asyncpg
import asyncio
from api.models.schemas import PortfolioRequest, TradeReq
...
async def get_portfolio_holdings(current_user: dict = Depends(get_current_user), db: asyncpg.Connection = Depends(get_db)):
    ...
    rows = await db.fetch(
        "SELECT ticker, sum(shares) as shares, sum(allocated_capital) / sum(shares) as avg_price, sum(allocated_capital) as invested FROM portfolios WHERE user_id = $1 GROUP BY ticker",
...
async def execute_trade(req: TradeReq, current_user: dict = Depends(get_current_user), db: asyncpg.Connection = Depends(get_db)):
    ...
    for item in req.basket:
        ...
        await db.execute(
            ...
            current_user["user_id"], ticker, shares, price, shares * price
        )
